...
// ---------------------------------------------------------------------------
// 4D Simplex noise — inlined from webgl-noise/src/noise4D.glsl
...
void main() {
    float n = snoise(vec4(v_coord, pc.time));
    n += 0.5 * snoise(vec4(v_coord * 2.0, pc.time * 1.4));
    n += 0.25* snoise(vec4(v_coord * 4.0, pc.time * 2.0));
    n *= 0.7;
