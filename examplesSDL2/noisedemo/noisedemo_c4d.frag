...
// ---------------------------------------------------------------------------
// 4D Classic Perlin noise — inlined from webgl-noise/src/classicnoise4D.glsl
...
float cnoise(vec4 P) {
    vec4 Pi0 = floor(P), Pi1 = Pi0 + 1.0;
    Pi0 = mod289(Pi0); Pi1 = mod289(Pi1);
    vec4 Pf0 = fract(P), Pf1 = Pf0 - 1.0;
    vec4 ix  = vec4(Pi0.x,Pi1.x,Pi0.x,Pi1.x);
    vec4 iy  = vec4(Pi0.yy, Pi1.yy);
    vec4 iz0 = vec4(Pi0.zzzz), iz1 = vec4(Pi1.zzzz);
    vec4 iw0 = vec4(Pi0.wwww), iw1 = vec4(Pi1.wwww);
    vec4 ixy  = permute(permute(ix) + iy);
    vec4 ixy0 = permute(ixy + iz0), ixy1 = permute(ixy + iz1);
    vec4 ixy00 = permute(ixy0+iw0), ixy01 = permute(ixy0+iw1);
    vec4 ixy10 = permute(ixy1+iw0), ixy11 = permute(ixy1+iw1);
    vec4 gx00=ixy00*(1.0/7.0), gy00=floor(gx00)*(1.0/7.0), gz00=floor(gy00)*(1.0/6.0);
    ...
    vec4 gx01=ixy01*(1.0/7.0), gy01=floor(gx01)*(1.0/7.0), gz01=floor(gy01)*(1.0/6.0);
    ...
    vec4 gx10=ixy10*(1.0/7.0), gy10=floor(gx10)*(1.0/7.0), gz10=floor(gy10)*(1.0/6.0);
    ...
    vec4 gx11=ixy11*(1.0/7.0), gy11=floor(gx11)*(1.0/7.0), gz11=floor(gy11)*(1.0/6.0);
...
void main() {
    float n = cnoise(vec4(v_coord, pc.time));
    n += 0.5 * cnoise(vec4(v_coord * 2.0, pc.time * 1.4));
    n += 0.25* cnoise(vec4(v_coord * 4.0, pc.time * 2.0));
    n *= 0.7;
