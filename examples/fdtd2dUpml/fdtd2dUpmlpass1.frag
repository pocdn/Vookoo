...
void mainImage( out vec4 ep, out vec4 dp, in vec2 fragCoord ){
    ...
    if (2<=i && i<=ie_tot && 2<=j && j<=je_tot) {
      ...
      vec2 r = vec2(i,j) - vec2(is,js);
      float dist = sqrt(dot(r,r));
      if (dist<1.5) {
        float f = 5. + (10.-5.)*(1.-float(iFrame%4000)/float(4000));
        dp.z += J0*sin((2.*3.14*f*1e9)*(float(iFrame%4000)*dt));
        //float n = float(iFrame);
        //dp.z += J0*(n-ndelay)*exp(-pow((n-ndelay)/tau,2.));
      };
